...
export async function getStaticPaths() {
  const paths = musicStore.dj_equipment.items.map((item) => ({
    params: { djItem: item.name }, // Use item name directly without encoding
  }));
...
export async function getStaticProps({ params }) {
  const itemName = params.djItem;
  const item = musicStore.dj_equipment.items.find((i) => i.name === itemName);
