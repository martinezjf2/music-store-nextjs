...
export async function getStaticPaths() {
  const paths = musicStore.keyboards.items.map((item) => ({
    params: { keyboardItem: item.name }, // Use item name directly without encoding
  }));
...
export async function getStaticProps({ params }) {
  const itemName = params.keyboardItem;
  const item = musicStore.keyboards.items.find((i) => i.name === itemName);
  ...
  return {
    props: {
      item,
      storeName: musicStore.keyboards.name,
    },
  };
}
